...
extend(Nut.prototype, {
  ...
  _apidoc: function (args) {
    ...
    return this._command('apidoc', args);
  },
  apidoc: function () {
    if( this.opts.apidoc ){
      if( this.opts.help ){
  ...
  _rjs: function (opts) {
    ...
    return new Promise(function (resolve, reject) {
      requirejs.optimize(opts, function (buildResponse) {
        ...
        this.log(buildResponse.replace(/^/mg, "  "));
        resolve();
    ...
    return new Promise(function (resolve, reject) {
      ...
      .then(function () {
        ...
        this._rjs({
          ...
          resolve(this.modifyIndex());
        }.bind(this));
      }.bind(this))
      .catch(function (err) {
        // We use stat to abort if the file doesn't exist, so simplify the message.
        if( err.code === 'ENOENT' ){
          reject(new Error('Cannot optimize; not found: "' + main + '"'));
        }
        else {
          reject(err);
        }
  ...
  modifyFile: function (file, contentcb) {
    ...
    return new Promise(function (resolve) {
      pfs.readFile(file, fileopts)
        .then(function (content) {
          return contentcb(content);
        })
        .then(function (content) {
          resolve( pfs.writeFile(file, content, fileopts) );
        });
    });
  },
