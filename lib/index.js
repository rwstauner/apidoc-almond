...
extend(Nut.prototype, {
  ...
  _command: function (cmdpath, args) {
    ...
    return new Promise(function (resolve) {
      ...
      resolve(require.resolve(cmdpath));
    })
    .catch(function (err) {
    ...
    .then(function (cmd) {
      return new Promise(function (resolve, reject) {
        ...
        // Track this since 'on close' could still run after rejection.
        var errored = false;
        ...
        .on('error', function (err) {
          ...
          if( err.code === 'ENOENT' ){
            reject(new Error(sprintf('Command "%s" not found.  Is "%s" installed?',
              cmdbase, cmdmod
            )));
          }
          // Rethrow.
        ...
        .on('close', function (code) {
          ...
          if( code !== 0 && !errored ){
            var err = new Error(sprintf('Command "%s" exited with %s', cmdbase, code));
            err.exitCode = code;
            reject(err);
          }
          resolve();
  ...
  _rjs: function (opts) {
    ...
    return new Promise(function (resolve, reject) {
      requirejs.optimize(opts, function (buildResponse) {
        ...
        reject(new Error("RequireJS optimization failed: " + err.message));
      });
    }.bind(this));
  },
...
Nut.main = function (opts) {
  return new Nut(opts).run()
    // If an error occurs running the command, make sure we display it.
    .catch(function (err) {
      if( err.exitCode ){
